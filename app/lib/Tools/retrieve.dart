...
Future<BankAccount> getBankAccount(User userObj) async {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      bankAccounts = (json.decode(response.body) as List).map((i) => BankAccount.fromJson(i)).toList();
...
Future<List<Operation>> getOperations(User userObj) async {
  ...
  try {
    var response = await client.get(
        Uri.https(env["URL_PROD"].toString(), "/users/" + userObj.email + "/banks/1/operations"),
        headers: {HttpHeaders.authorizationHeader: "Bearer ${userObj.token}"});
    if (response.statusCode == 200) {
      List<Operation> operations;
      operations = (json.decode(response.body) as List).map((i) => Operation.fromJson(i)).toList();
      operations = operations.where((x) => x.transfer == true || (x.invoice == true && x.acquitted == true)).toList();
...
Future<List<Invoice>> getInvoices(User userObj) async {
  ...
  try {
    var response = await client.get(
        Uri.https(env["URL_PROD"].toString(), "/users/" + userObj.email + "/banks/1/invoices"),
        headers: {HttpHeaders.authorizationHeader: "Bearer ${userObj.token}"});
    if (response.statusCode == 200) {
